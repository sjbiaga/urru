...
object Term:
  ...
  extension(self: Terminal)
    ...
    def prompt(game: Game, paused: Boolean, idleTime: Long): Unit =
      ...
      self.setCursorPosition(0, 2*(size.row+1)+2)
      ...
      self.setCursorPosition(0, 2*(size.row+1)+3)
      self.setForegroundColor(Code.colors(1))
      ...
      then
        ...
        do
          self.setCursorPosition(0, 2*(size.row+1)+4)
          self.setForegroundColor(Code.colors(1))
...
/*
                      ( for
                          ...
                          _ <- doneD.get
                          _ <- IO {
                            self.setCursorPosition(0, 2*(game.size.row+1)+5)
                            self.setForegroundColor(Code.colors(1))
            ...
            _ <-  ( if keyStroke.getKeyType eq KeyType.Escape
                    then
                      exitR.set(true) >> IO {
                        ...
                        self.setForegroundColor(Code.colors(0))
                        self.putString("Use TAB to switch colors.")
                        println()
                        self.putString("Use symbols <, >, ^, v to (un)fold left, right, up, down.")
                        println()
                      ...
                      ( if paused
                        then
                          ...
                          then
                            ...
                            then
                              IO { game.showJust = None } >> render(clues, justR, linesR)

                            else if chr == '*'
                            then
                              IO { game.showJust = game.showJust.map(!_).orElse(Some(true)) } >> render(clues, justR, linesR)
